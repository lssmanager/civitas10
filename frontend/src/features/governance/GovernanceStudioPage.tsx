...
import { BlockCard, CapabilityCard, isInvalidOrganizationId } from "../../pages/OwnerOrganizationOperationalPage";
...
const OperationalOverview = ({ organization }: { organization: ConsolidatedOperationalResponse }) => (
  <section className="grid gap-4 md:grid-cols-3">
    <MetricCard label="Summary" detail={organization.summary.humanMessage || "Capability surface loaded."}><OwnerBadge tone={ownerToneFromSeverity(organization.summary.severity || "info")}>{organization.summary.status || "available"}</OwnerBadge></MetricCard>
...
const OperationalModules = ({ organization }: { organization: ConsolidatedOperationalResponse }) => (
  <>
    <section id="operations" className="grid gap-4 md:grid-cols-4"><MetricCard label="Summary" detail={organization.summary.humanMessage || "Capability surface loaded."}><OwnerBadge tone={ownerToneFromSeverity(organization.summary.severity || "info")}>{organization.summary.status || "available"}</OwnerBadge></MetricCard><MetricCard label="Capabilities" value={organization.capabilities.length} detail="Owner capability surface returned by the backend contract." /><MetricCard label="Blockers" value={organization.blockers.length} detail="Aggregated capability blockers." /><MetricCard label="Polling" value={organization.polling.shouldPoll ? `${organization.polling.intervalSeconds}s` : "stopped"} detail={organization.polling.reason || "-"} /></section>
    <section className="grid gap-4 lg:grid-cols-2">{organization.capabilities.map((capability) => <CapabilityCard key={capability.capability} capability={capability} />)}{organization.worker ? <BlockCard title="Worker" block={organization.worker} /> : null}</section>
  </>
);
