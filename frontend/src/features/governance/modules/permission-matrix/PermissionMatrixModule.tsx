...
const BooleanCell = ({ value }: { value: boolean | null }) => {
  if (value === null) return <span className="font-mono text-slate-400" aria-label="not applicable">—</span>;
...
const ReasonCell = ({ reason }: { reason: PermissionMatrixReason }) => (
  <div>
    ...
    <div className="text-xs font-mono text-slate-500">{formatSourceVersions(reason.sourceVersions)}</div>
  </div>
);
  ...
  <SectionCard title={surface === "owner" ? "Roles and permission ceilings" : "Active permissions"} description="Columns remain distinct: canonical, role potential, owner allowed, tenant enabled, effective and first denial reason with source versions.">
    <div className="overflow-x-auto">
      <table className="min-w-full text-left text-sm" data-governance-permission-matrix="true">
        <thead><tr>{["Permission", "Canonical", "Role potential", "Owner allowed", "Tenant enabled", "Effective", "Reason"].map((header) => <th key={header} className="px-3 py-2 font-semibold text-slate-700">{header}</th>)}</tr></thead>
        <tbody>
          {rows.map((row) => <tr key={row.permission} data-reason-code={row.reason.code}>
            <td className="px-3 py-2 font-medium text-slate-900">{row.permission}</td>
            ...
            <td className="px-3 py-2 text-slate-600"><ReasonCell reason={row.reason} /></td>
          </tr>)}
        </tbody>
      </table>
    </div>
    {rows.length === 0 ? <p className="text-sm text-slate-600">No governance matrix rows were returned by the aggregate read model.</p> : null}
  </SectionCard>
);
