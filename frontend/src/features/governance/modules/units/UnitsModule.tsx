...
const descendantsOf = (units: readonly GovernanceUnitItem[], unitId: string): Set<string> => {
  ...
  const visit = (id: string) => units.filter((unit) => unit.parentId === id).forEach((child) => { if (!descendants.has(child.id)) { descendants.add(child.id); visit(child.id); } });
  visit(unitId);
  return descendants;
};
