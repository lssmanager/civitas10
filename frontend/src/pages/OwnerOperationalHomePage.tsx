      ...
      <main className="mx-auto flex max-w-7xl flex-col gap-6 px-4 py-8 sm:px-6 lg:px-8">
        <section className={cardClass}>
          <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <div>
              <p className="text-sm font-semibold uppercase tracking-[0.2em] text-blue-600">Owner backbone</p>
              <h1 className="mt-2 text-3xl font-semibold tracking-tight text-slate-950">Operational overview</h1>
              <p className="mt-2 max-w-3xl text-sm text-slate-600">Esta vista nueva se apoya en el contrato operacional consolidado y en una lectura limpia del runtime. No usa logs legacy como verdad principal.</p>
            </div>
            <Link to={appRoutes.ownerOrganizations.path} className="inline-flex items-center rounded-xl bg-slate-950 px-4 py-3 text-sm font-semibold text-white transition hover:bg-slate-800">Create organization</Link>
          </div>
        </section>
        ...
        <section className="grid gap-4 md:grid-cols-3">
          ...
          <article className={cardClass}>
            <p className="text-sm text-slate-500">Worker health</p>
            ...
            <p className="mt-3 text-sm text-slate-600">{runtime?.workerHealth.humanMessage || "Worker health not loaded yet."}</p>
          </article>
          <article className={cardClass}>
            ...
            <Link to={appRoutes.ownerWorkerQueues.path} className="mt-3 inline-flex text-sm font-medium text-blue-700 hover:text-blue-900">Open runtime view</Link>
          </article>
        </section>
        ...
        <section className={cardClass}>
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-600">Cada fila abre el `operational-state` limpio de esa organización.</p>
            </div>
          </div>
            ...
            <table className="min-w-full divide-y divide-slate-200 text-sm">
              <thead className="bg-slate-100 text-left text-slate-600">
                <tr>
                  ...
                  <th className="px-4 py-3 font-medium">CRM state</th>
              ...
              <tbody className="divide-y divide-slate-200 bg-white">
                {organizations.map((organization) => {
                  ...
                  const crmState = String(profile.fluentcrmSyncStatus || "not_linked");
                  return (
                    <tr key={orgId || organization.name || Math.random()}>
                      ...
                      <td className="px-4 py-4"><span className={badgeClass(crmState === "linked" ? "success" : "warning")}>{crmState}</span></td>
                      <td className="px-4 py-4">
                        {orgId ? <Link to={`/owner/organizations/${encodeURIComponent(orgId)}`} className="font-medium text-blue-700 hover:text-blue-900">Open state</Link> : <span className="text-slate-400">Unavailable</span>}
                      </td>
                    </tr>
                  );
                })}
                {!loading && organizations.length === 0 ? (
                  <tr><td colSpan={4} className="px-4 py-8 text-center text-slate-500">No organizations found in Logto.</td></tr>
                ) : null}
              </tbody>
            </table>
          </div>
