    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="mx-auto flex max-w-7xl flex-col gap-6 px-4 py-8 sm:px-6 lg:px-8">
        <section className={cardClass}>
          ...
          <h1 className="mt-2 text-3xl font-semibold tracking-tight text-slate-950">Worker and queues</h1>
          <p className="mt-2 text-sm text-slate-600">Vista global del runtime operativo limpio en `civitas10`.</p>
        </section>
        ...
        <section className="grid gap-4 md:grid-cols-3">
          ...
          <article className={cardClass}><p className="text-sm text-slate-500">Queues</p><p className="mt-2 text-3xl font-semibold text-slate-950">{data?.queues.length ?? 0}</p></article>
          <article className={cardClass}><p className="text-sm text-slate-500">Blocked organizations</p><p className="mt-2 text-3xl font-semibold text-slate-950">{data?.blockedOrganizations.length ?? 0}</p></article>
        </section>
            ...
            <div className="mt-4 space-y-3">
              {data?.queues.map((queue) => (
                <div key={queue.name} className="rounded-xl border border-slate-200 p-4">
                  ...
                  <p className="mt-2 text-sm text-slate-600">waiting {queue.waiting} · active {queue.active} · delayed {queue.delayed} · failed {queue.failed}</p>
                </div>
              ))}
            </div>
          </article>
