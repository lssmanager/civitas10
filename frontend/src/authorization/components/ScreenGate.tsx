...
export const ScreenGate = ({ screenId, children }: { screenId: ScreenId | string; children: ReactNode }) => {
  ...
  if (context.status === "loading" || context.status === "idle") return <div className="p-6 text-sm text-muted-strong">Loading authorization...</div>;
  if (context.status === "unauthenticated") return <div className="p-6 text-sm text-muted-strong">Authentication required.</div>;
  const screen = visualRegistry.screenById.get(screenId as ScreenId);
  if (!screen) return <div className="p-6"><h1 className="text-2xl font-semibold text-text">404 / Screen unknown</h1></div>;
  ...
  return <div className="p-6"><h1 className="text-2xl font-semibold text-text">{title}</h1><p className="mt-2 text-sm text-muted-strong">{decision.reason}</p></div>;
};
