...
export const NavCollapse = ({ items, label, collapsed = false }: { items: NavCollapseItem[]; label: string; collapsed?: boolean }) => {
  ...
  const links = items.map((item) => {
    ...
    return (
      <Link key={`${item.label}-${item.path}`} to={item.path} className={`civitas-nav-link ${active ? "civitas-nav-link-active" : ""}`} title={collapsed ? item.label : undefined} aria-label={collapsed ? item.label : undefined}>
