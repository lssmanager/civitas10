...
- Backend `API_URL`: audience validated by `requireAuth(API_RESOURCE)`.
...
- Organization pages use `getOrganizationToken(organizationId)` only for tenant-scoped endpoints.
