...
function assertSharedContract(contract) {
  ...
  if (!resource || !/^https?:\/\//i.test(resource)) throw new Error("Invalid Civitas shared contract: logto.apiResource must be the canonical HTTP URL resource indicator");
  ...
  if (resource !== publicUrl) throw new Error("Invalid Civitas shared contract: Logto API resource must equal public API URL");
