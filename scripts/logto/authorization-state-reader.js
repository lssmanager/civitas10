...
function emptyRemoteState({ source = 'empty' } = {}) { return normalizeRemoteState({ source, status: source === 'empty-no-credentials' ? 'verification_required' : 'empty', resources: [], resource: null, permissions: [], globalRoles: [], organizationRoles: [] }) }
function normalizeRemoteState({ source = 'remote', status = 'verified', resources = [], resource = null, permissions = [], globalRoles = [], organizationRoles = [] } = {}) {
