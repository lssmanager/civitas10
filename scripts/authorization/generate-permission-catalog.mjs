...
const outputs = {'core/authz/catalog/generated/permission-catalog.js': runtime, 'artifacts/authorization/permission-catalog.json': inventory}
let drift = false
for (const [path, body] of Object.entries(outputs)) { if (fs.existsSync(path) && fs.readFileSync(path,'utf8') !== body) drift = true; if (!check) fs.writeFileSync(path, body) }
if (check && drift) { console.error('generated permission catalog artifacts are out of date'); process.exit(1) }
