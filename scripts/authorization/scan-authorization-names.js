...
function isIntegrationEventPublicContractBoundary(file, lineText) {
  ...
  // integration-event service/repository code, and module UI contract fixtures is excluded.
  return /organizationId/.test(lineText) && (
    ...
    file.startsWith('frontend/src/module-ui/')
  )
}
...
function scanFile(file, root = process.cwd()) {
  ...
  text.split(/\r?\n/).forEach((lineText, index) => {
    ...
    if (!isIntegrationEventPublicContractBoundary(file, lineText) && (/organization_id/.test(lineText) || ((file === 'backend/middleware/auth.js' || file === 'backend/authorization/guards.js' || file === 'backend/middleware/requireOrg.js' || file.includes('connectors/')) && /organizationId|org_id/.test(lineText)))) {
