...
function createHealthStatus({ status = HEALTH_STATUSES.HEALTHY, timestamp = Date.now(), details, latencyMs = 0, error } = {}) {
  if (!Number.isFinite(timestamp)) {
    throw contractError("HealthStatus timestamp must be a finite number", { timestamp });
  }
  if (latencyMs !== undefined && !Number.isFinite(Number(latencyMs))) {
    throw contractError("HealthStatus latencyMs must be numeric", { latencyMs });
  }
