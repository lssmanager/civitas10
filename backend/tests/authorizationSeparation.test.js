...
test("owner routes use global access while tenant routes keep organization access", () => {
  ...
  assert.match(source, /secureRoute\.post\("\/owner\/organization-drafts", "ownerSensitiveWrite", requireGlobalAccess\(\{ resource: API_RESOURCE, requiredScopes: \[OWNER_SCOPES.organizationCreate\] \}\), requireGlobalOwner/);
